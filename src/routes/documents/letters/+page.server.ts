...
export const actions = {
	create: async ({ request }) => {
		const { letterCode, letterType, letterGroup, tape, ruamail, owner, description, updatedAt } = Object.fromEntries(await request.formData()) as {
			letterCode: string;
			letterType: string;
			letterGroup: string;
			tape: string;
	...
	update: async ({ request }) => {
		const { id, letterType, letterGroup, tape, ruamail, owner, description, updatedAt } = Object.fromEntries(await request.formData()) as {
			id: string;
			letterType: string;
			letterGroup: string;
			tape: string;
