...
export const actions = {
	create: async ({ request }) => {
		const { fileName, aidYear, owner, rraareq, bdms, web, updatedAt } = Object.fromEntries(await request.formData()) as {
			fileName: string;
			letterType: string;
			aidYear: string;
			owner: string;
	...
	update: async ({ request }) => {
		const { id, fileName, aidYear, owner, rraareq, bdms, web, updatedAt } = Object.fromEntries(await request.formData()) as {
			id: string;
			fileName: string;
			letterType: string;
			aidYear: string;
