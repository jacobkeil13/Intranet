						...
						<section class="p-6 rounded-md border border-accSlate/20">
								...
								{#if hasUID}
									<section class="flex items-center space-x-3">
										<div class="relative w-fit">
											<div class="input-group input-group-divider bg-transparent rounded-md grid-cols-[auto_1fr_auto]">
							...
							<Step locked={$appointmentForm.reason === ''}>
								...
								{#if $appointmentForm.reason !== ''}
									<div class:border-usfGreen={$appointmentForm.submittedDocument} class="card bg-transparent flex items-center space-x-3 p-4 w-fit border border-accSlate/50">
										<h1>Visitor submitted a document</h1>
										<SlideToggle name="document" size="sm" bind:checked={$appointmentForm.submittedDocument} />
									</div>
									<div class:border-usfGreen={$appointmentForm.needsFutherAction} class="card bg-transparent flex items-center space-x-3 p-4 w-fit border border-accSlate/50">
									...
									{#if $appointmentForm.needsFutherAction && currentUser?.campus === 'P'}
										<div class="flex gap-2">
											<div class:border-usfGreen={$appointmentForm.stPeteAppt} class="card bg-transparent flex items-center space-x-3 p-4 w-fit border border-accSlate/50">
							...
							{#if $appointmentForm.reason !== '' && $appointmentForm.needsFutherAction && !$appointmentForm.stPeteAppt}
								<Step locked={action === ''} regionContent="space-y-0">
							...
							{#if action !== '' && $appointmentForm.needsFutherAction}
								<Step>
									...
									<div class="flex space-x-2">
										{#if action !== Actions.Referral}
											<input class="input bg-transparent rounded-md w-fit" type="date" name="date" id="date" min={minDate} max={maxDate} bind:value={$appointmentForm.date} />
										{:else}
									...
									<div class="flex space-x-2">
										{#if action !== Actions.Walkin}
											<div class="space-y-1">
												...
												<input
													class="input rounded-md bg-transparent w-fit"
													type="tel"
													name="phone"
													id="phone"
											...
											<div class="space-y-1">
												...
												<TextareaCopy bind:content={$appointmentForm.referralDetails}>
													<svelte:fragment slot="textarea">
														<textarea
															required
															class="input bg-transparent rounded-md min-h-[100px] max-h-[150px] max-w-[500px]"
															name="description"
															cols="100"
															rows="4"
									...
									<div class="flex space-x-2">
										<div class:border-usfGreen={$appointmentForm.rhacomm} class="card bg-transparent flex items-center space-x-3 p-4 w-fit border border-accSlate/50">
