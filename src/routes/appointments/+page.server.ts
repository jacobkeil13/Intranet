import { email, getUtcDate } from '$lib/helpers';
...
export const actions = {
	create: async ({ locals, request }) => {
		const { type, visitorType, studentUid, studentEmail, studentName, studentCampus, appReason, rhacomm, callbackNumber, advisorRequested, date, time, source } = Object.fromEntries(
	...
	update: async ({ locals, request }) => {
		const { id, timeIn, timeOut, complete, advisor } = Object.fromEntries(await request.formData()) as {
			id: string;
			timeIn: string;
			timeOut: string;
			complete: string;
