...
export const actions = {
	create: async ({ locals, request }) => {
		const { firstName, lastName, netId, uidRange, directReport, userType } = Object.fromEntries(await request.formData()) as {
			firstName: string;
			lastName: string;
			netId: string;
			uidRange: string;
		...
		try {
			const tempUser = await db.userProfile.create({
				data: {
					...
					team: {
						connect:
							directReportUser?.team.length !== 0
								? directReportUser?.team.map((team) => {
	...
	update: async ({ locals, request }) => {
		const { id, firstName, lastName, netId, uidRange, directReport } = Object.fromEntries(await request.formData()) as {
			id: string;
			firstName: string;
			lastName: string;
			netId: string;
