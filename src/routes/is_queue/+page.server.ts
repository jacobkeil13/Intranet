  ...
  create: async ({ locals, request }) => {
    ...
    try {
      ...
      additionalUsers.forEach(user => {
        let userProfile = allUsers.filter(usr => usr.id === user.meta.id)
        cc.push(userProfile[0].netid + "@usf.edu");
      })

      isUsers = isUsers.filter(user => !user.startsWith("tbd"));
      if (cc) { cc = cc.filter(user => !user?.startsWith("tbd")); }
      ...
      await email("request", {
        ...
        "assignedUser": newQueueItemEmail?.assignedTo === null ? "None" : 
        newQueueItemEmail?.assignedTo?.first_name + " " + newQueueItemEmail?.assignedTo?.last_name,
      ...
      if (complete === "on") {
        if (updatedQueueItem.populationSelectionId !== null) {
          await email("request", {
            ...
            "assignedUser": updatedQueueItemEmail?.assignedTo === null ? "None" : 
            updatedQueueItemEmail?.assignedTo?.first_name + " " + updatedQueueItemEmail?.assignedTo?.last_name,
