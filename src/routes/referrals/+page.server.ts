...
export const actions = {
        ...
        include: {
          comments: {
			...
			if (newReferralData !== null && newReferralData.researchUser !== null) {
				allCollaborators = newReferralData.researchUser.split(',').map(user => {
  ...
  update: async ({ locals, request }) => {
      ...
      id,
		...
		if (!collaborators.split(',').includes(counterUserInfo.first_name + " " + counterUserInfo.last_name) && 
			counterUser !== counterUserInfo.first_name + " " + counterUserInfo.last_name) 
		{
			...
			if (complete === "on") {
				await email("referral", {
					...
					"owner": updatedReferral?.escalationUser !== null && updatedReferral?.referralType === "Escalated Referral" ? 
						updatedReferral?.escalationUser : ownerInfo[0].first_name + " " + ownerInfo[0].last_name,
