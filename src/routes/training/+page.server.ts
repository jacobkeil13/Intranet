...
export const actions = {
	createTraining: async ({ request }) => {
		const { title, date, trainerList } = Object.fromEntries(await request.formData()) as {
			title: string;
			date: string;
			trainerList: string;
		};
		...
		try {
			...
			const newTrainingSechedule = await db.trainingSchedule.create({
				data: {
					...
					trainers: {
						connect:
							trainerListArr.length !== 0
								? trainerListArr.map((user: AutocompleteOption) => {
	...
	updateTraining: async ({ request }) => {
		const { id, title, date, trainerList } = Object.fromEntries(await request.formData()) as {
			id: string;
			title: string;
			date: string;
			trainerList: string;
		...
		try {
			...
			const updatedTraining = await db.trainingSchedule.update({
				...
				data: {
					...
					date: dateDueConvert,
					trainers: {
						set: trainerListArr.length !== 0 ? trainerListArr.map((user: AutocompleteOption) => ({ id: (user.meta as { id: string }).id })) : []
					},
					weekday
				}
			});
	...
	createLibrary: async ({ request }) => {
		const { title, date, url, trainerList } = Object.fromEntries(await request.formData()) as {
			title: string;
			date: string;
			url: string;
			trainerList: string;
	...
	updateLibrary: async ({ request }) => {
		const { id, title, date, url, trainerList } = Object.fromEntries(await request.formData()) as {
			id: string;
			title: string;
			date: string;
			url: string;
