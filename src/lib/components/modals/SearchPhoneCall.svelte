<script lang="ts">
  ...
  $: {
    filteredCalls = calls.filter((call) => {
      if (call.split(" ")[0].toLowerCase().includes(searchQuery.toLowerCase().trim()) ||
          getUser(call.split(" ")[2].split("@")[0]).toLowerCase().includes(searchQuery.toLowerCase().trim())) {
        return call;
      }
    })
	...
	$: paginatedSource = filteredCalls.slice(
...
<section class="w-[50rem] max-h-[calc(100%_-_5rem)] overflow-y-auto bg-usfWhite p-4 rounded-md">
  ...
  {#if currentAudio && currentAudioDetails}
    <div>
      <h1 class="mb-2">{parsePhoneNumber(currentAudioDetails.split(" ")[0])} - {getUser(currentAudioDetails.split(" ")[2].split("@")[0])} 
        at {parseCallTime(currentAudioDetails.split(" ")[4])}{currentAudioDetails.split(" ")[5].split(".")[0]}</h1>
      <audio class="w-full border-2 border-[#4C4C4C] bg-accSlate rounded-full" controls autoplay src={currentAudio}></audio>
    </div>
  {/if}
  <br>
  <TableWrapper arrLength={filteredCalls.length} bind:paginationSettings={paginationSettings} tableCompact="table-compact">
    ...
    <svelte:fragment slot="body">
      <tbody>
        {#each paginatedSource as call}
          <tr 
            on:click={() => { playAudio(`https://tup-ofa.forest.usf.edu/finaid/files/phone/${moment().format("M_D_YYYY")}/${call}?apiKey=API_KEY`) }} 
